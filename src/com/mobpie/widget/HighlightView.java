...
package com.mobpie.widget;


import com.mobpie.instagram.R;
...
public class HighlightView {
    ...
    public static int MinHeight = 220;
    ...
    private void init() {
        ...
        mResizeDrawableNormal = resources.getDrawable(R.drawable.button_crop);
        mResizeDrawableTouch = resources.getDrawable(R.drawable.button_crop_touch);
    }
...
//                mOutlinePaint.setColor(0xFFFF8A00);
            ...
            RectF rectF = new RectF(mImageRect.left, mImageRect.top, mImageRect.right,
            		mImageRect.bottom);
...
//                    : mNoFocusPaint);
            canvas.drawRect(rectF, hasFocus() ? mFocusPaint
                    : mNoFocusPaint);
            ...
            if(mMode == ModifyMode.Grow)
            {
                ...
                int top = mDrawRect.top + 1;
                int bottom = mDrawRect.bottom + 1;
                
                int touchWidth = mResizeDrawableTouch.getIntrinsicWidth() / 2;
                int touchHeight = mResizeDrawableTouch.getIntrinsicHeight() / 2;
                
                mResizeDrawableTouch.setBounds(left-touchWidth, top-touchHeight,
                		left + touchWidth, top + touchHeight);
                mResizeDrawableTouch.draw(canvas);
                mResizeDrawableTouch.setBounds(left-touchWidth, bottom-touchHeight,
                		left + touchWidth, bottom + touchHeight);
                mResizeDrawableTouch.draw(canvas);
                mResizeDrawableTouch.setBounds(right-touchWidth, top-touchHeight,
                		right + touchWidth, top + touchHeight);
                mResizeDrawableTouch.draw(canvas);
                mResizeDrawableTouch.setBounds(right-touchWidth, bottom-touchHeight,
                		right + touchWidth, bottom + touchHeight);
                mResizeDrawableTouch.draw(canvas);
                ...
                int touchWidth = mResizeDrawableNormal.getIntrinsicWidth() / 2;
                int touchHeight = mResizeDrawableNormal.getIntrinsicHeight() / 2;
                
                mResizeDrawableNormal.setBounds(left-touchWidth, top-touchHeight,
                		left + touchWidth, top + touchHeight);
                mResizeDrawableNormal.draw(canvas);
                mResizeDrawableNormal.setBounds(left-touchWidth, bottom-touchHeight,
                		left + touchWidth, bottom + touchHeight);
                mResizeDrawableNormal.draw(canvas);
                mResizeDrawableNormal.setBounds(right-touchWidth, top-touchHeight,
                		right + touchWidth, top + touchHeight);
                mResizeDrawableNormal.draw(canvas);
                mResizeDrawableNormal.setBounds(right-touchWidth, bottom-touchHeight,
                		right + touchWidth, bottom + touchHeight);
                mResizeDrawableNormal.draw(canvas);
			}
    ...
    void handleMotion(int edge, float dx, float dy) {
        ...
        if (edge == GROW_NONE) {
            ...
            if(dx!=0 && dy!=0)
            {
            	if((edge&GROW_LEFT_EDGE) == 0)
            	{
            		...
            		if((edge&GROW_TOP_EDGE) == 0)
            		{
                    	...
                    	{
                    		yDelta=xDelta/mInitialAspectRatio;
                    	}
            			else {
            				xDelta = yDelta*mInitialAspectRatio;
						}
            ...
            else
            {
	            ...
	            int xx = ((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1;
	            int yy = ((edge & GROW_TOP_EDGE) != 0) ? -1 : 1;
	            if(xx<0||yy<0)
	            {
	            	xDelta = -xDelta;
	            	yDelta = -yDelta;
...
//            if(xx<0||yy<0)
//            {
//            	growBy(-yDelta, -yDelta);
//            }
//            else {
//				growBy(yDelta, yDelta);
//			}
//            growBy((((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1) * yDelta,
...
//        }
        ...
        //��С����
        if(dx < 0F && r.width() + 2 * dx < MinWidth )
        {
        	float adjustment = (MinWidth - r.width()) / 2F;
        	dx = adjustment;
        	if (mMaintainAspectRatio) {
                dy = dx / mInitialAspectRatio;
            }
        }
        if(dy < 0F && r.height() + 2 * dy < MinHeight )
        {
        	float adjustment = (MinHeight - r.height()) / 2F;
        	dy = adjustment;
        	if (mMaintainAspectRatio) {
        		dx = dy * mInitialAspectRatio;
            }
...
//    
    private Drawable mResizeDrawableNormal;
    private Drawable mResizeDrawableTouch;
